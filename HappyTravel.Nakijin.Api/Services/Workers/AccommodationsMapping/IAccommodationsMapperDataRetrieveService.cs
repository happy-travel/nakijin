...
namespace HappyTravel.Nakijin.Api.Services.Workers.AccommodationsMapping
{
    ...
    {
        Task<List<Contracts.MultilingualAccommodation>> GetAccommodationsForMapping(string countryCode,
            Suppliers supplier, int skip, int take, DateTime lastUpdatedDate, CancellationToken cancellationToken);


        Task<List<(string SupplierCode, SlimAccommodationData AccommodationKeyData)>> GeCountryAccommodationBySupplier(string countryCode,
            Suppliers supplier);
        ...
        Task<List<Tuple<int, int>>> GetActiveCountryUncertainMatchesBySupplier(string countryCode,
        ...
        Task<Dictionary<(int LocalityId, string LocalityZoneName), int>> GetLocalityZonesByCountry(
            int countryId);
        ...
        Task<STRtree<SlimAccommodationData>> GetCountryAccommodationsTree(string countryCode,
            Suppliers supplier);
    }
}